...
function pemicu_telfon() {
    return new Promise((resolve) => {
        ...
        exec(`adb -s localhost:5555 shell am force-stop com.samsung.android.dialer`, () => {
            ...
            exec(cmd_telfon, (err) => {
                if (err) {
                    log.error("Gagal membuka aplikasi telfon", err.message);
                    resolve(false);
                    return;
                }
                ...
                setTimeout(() => {
                    ...
                    exec(`adb -s localhost:5555 shell input text ${nomor_darurat}`, (err) => {
                        ...
                        setTimeout(() => {
                            log.info("Menekan tombol telfon (CALL)...");
...
// 🟢 Fungsi baru yang menjalankan keduanya secara simultan
async function pemicuSimultan() {
    log.alert("Menjalankan telepon dan WhatsApp secara BERSAMAAN...");

    const [statusTelfon, statusWA] = await Promise.all([
        pemicu_telfon(),
        pemicu_pesan()
    ]);

    log.info(`Selesai — Telfon: ${statusTelfon ? 'berhasil' : 'gagal'}, WA: ${statusWA ? 'berhasil' : 'gagal'}`);
    return { statusTelfon, statusWA };
}

module.exports = { pemicu_telfon, pemicu_pesan, pemicuSimultan };